...
| **1.13.5** | Re-bind a SkeletalMesh onto another Skeleton — closes the `skeletal-mesh-set-skeleton` spec request. `SkeletalMeshSetSkeleton(Mesh, TargetSkeleton, missing_bone_policy)` is the editor's *Skeleton → Assign Skeleton*, headless: a mesh authored for skeleton A can natively play skeleton B's animations (no IK-retarget, no leader-pose) — every shared-name bone lands exactly where B's anim puts it. Maps the mesh ref-skeleton bones to the target by name; bones the target lacks follow `missing_bone_policy`: **fail** (abort + report unmatched), **merge_into_target** (default — `USkeleton::MergeAllBonesToBoneTree` grows the target's bone tree so the mesh keeps every joint), **remap_to_parent** (`IMeshUtilities::RemoveBonesFromMesh` collapses each unmatched bone's skin weights onto its nearest surviving ancestor and drops the bone — the engine's own weight-to-parent collapse + re-normalization, not hand-rolled buffer surgery). Then `USkeletalMesh::SetSkeleton` + `PostEditChange` rebuilds render data; bind-pose geometry is untouched (re-target, not reshape). Returns a JSON descriptor `{matched, merged, remapped, target_modified, unmatched:[{bone, action}], new_skeleton}`; the Python helper saves the mesh (and the target skeleton if it grew). New `Projects`-adjacent module dependency: `MeshUtilities`. One new MCP tool: `skeletal_mesh_set_skeleton`. Build.cs +MeshUtilities. |

...
| **1.10.2** | Bug fix for `bp_node_remove`. UE's `Graph->RemoveNode` broadcasts `OnGraphChanged`, which fires compile-on-modify listeners; one of those runs an "unreferenced variable" cleanup that silently swept member variables the user had declared but not yet wired. Fix: snapshot `UBlueprint::NewVariables` before the remove and restore anything that disappears (only restores names that were present before — never invents new vars). Logs a warning when a restore fires so the swept-by path is traceable. Closes the v1.10.1 follow-up bug report. |
